...
class BuildBuddyVi(
    private val buildBuddyContext: Context,
    private val buildBuddyCallback: BuildBuddyCallBack,
    private val buildBuddyWindow: Window
) : WebView(buildBuddyContext) {
    private var buildBuddyFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun buildBuddySetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.buildBuddyFileChooserHandler = handler
    }
    ...
    init {
        ...
        webSettings.apply {
            ...
            userAgentString =
                WebSettings.getDefaultUserAgent(buildBuddyContext).replace("; wv)", "")
                    .replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        buildBuddyContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(
                            buildBuddyContext,
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("ninecasino") == true) {
                    BuildBuddyApplication.buildBuddyInputMode =
                        WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(
                        BuildBuddyApplication.BUILD_BUDDY_MAIN_TAG,
                        "onPageFinished : ${BuildBuddyApplication.buildBuddyInputMode}"
                    )
                    buildBuddyWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    buildBuddyEnableStableInputScroll()
                } else {
                    BuildBuddyApplication.buildBuddyInputMode =
                    ...
                    buildBuddyWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                buildBuddyFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            ...
            override fun onCreateWindow(
                ...
                buildBuddyHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }

    // ГИБРИДНАЯ ФУНКЦИЯ - объединяет лучшее из обеих подходов
    private fun buildBuddyEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    fun buildBuddyFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun buildBuddyHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView =
                BuildBuddyVi(buildBuddyContext, buildBuddyCallback, buildBuddyWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            buildBuddyCallback.buildBuddyHandleCreateWebWindowRequest(windowWebView)
        }
    }
